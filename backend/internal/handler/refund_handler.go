...
// RefundHandler 退款登记处理层。
type RefundHandler struct {
	service *service.RefundService
	logger  *slog.Logger
}

// NewRefundHandler 构造退款登记处理层。
...
// @Param request body dto.CreateRefundRequest true "退款登记请求"
...
func (h *RefundHandler) List(c *gin.Context) {
	...
	refunds, err := h.service.ListByExpense(context.Background(), id)
	if err != nil {
		handleError(c, err)
		return
	}
